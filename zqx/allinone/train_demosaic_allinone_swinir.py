...
from data.demosaic_allinone_dataset import (
    AllInOneDemosaicEvalDataset,
    AllInOneDemosaicTrainDataset,
    VALID_BASE_PATTERNS,
    get_common_period,
)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train an all-in-one SwinIR demosaicing model for single/quad/nona patterns.")
    ...
    parser.add_argument("--save-dir", type=str, default="experiments/demosaic_allinone_swinir")
    parser.add_argument("--base-pattern", type=str, default="RGGB", choices=VALID_BASE_PATTERNS)
    parser.add_argument("--train-pattern-types", nargs="+", default=["single", "quad", "nona"])
    parser.add_argument("--train-pattern-weights", type=float, nargs="*", default=None)
    parser.add_argument("--val-pattern-types", nargs="+", default=["single", "quad", "nona"])
    ...
    parser.add_argument("--noise-std-max", type=float, default=0.0)
    parser.add_argument("--maskout-enabled", action="store_true")
    parser.add_argument("--maskout-p-min", type=float, default=0.0)
    parser.add_argument("--maskout-p-max", type=float, default=0.05)
...
def align_crop_size(crop_size: int, pattern_types: List[str], window_size: int) -> int:
    required_period = math.lcm(get_common_period(pattern_types), window_size)
...
def build_dataloaders(args):
    ...
    maskout_prob_range = (args.maskout_p_min, args.maskout_p_max)

    train_loader = None
    if args.train_dirs:
        train_dataset = AllInOneDemosaicTrainDataset(
            roots=args.train_dirs,
            crop_size=args.crop_size,
            pattern_types=args.train_pattern_types,
            pattern_weights=args.train_pattern_weights,
            base_pattern=args.base_pattern,
            noise_std_range=noise_std_range,
            maskout_enabled=args.maskout_enabled,
            maskout_prob_range=maskout_prob_range,
            augment_data=True,
            repeat=args.repeat,
        )
    ...
    val_loader = None
    if args.val_dirs:
        val_dataset = AllInOneDemosaicEvalDataset(
            roots=args.val_dirs,
            pattern_types=args.val_pattern_types,
            base_pattern=args.base_pattern,
            noise_std_range=noise_std_range,
        )
...
def validate(model: nn.Module, val_loader: DataLoader, device: torch.device):
    model.eval()
    per_pattern = defaultdict(lambda: {"psnr": [], "ssim": []})

    for batch in val_loader:
        inp = batch["input"].to(device, non_blocking=True)
        target = batch["target"].to(device, non_blocking=True)
        pattern_type = batch["pattern_type"][0] if isinstance(batch["pattern_type"], list) else batch["pattern_type"]
        ...
        target_img = tensor_to_uint8_image(target[0])

        per_pattern[pattern_type]["psnr"].append(calculate_psnr_np(pred_img, target_img))
        if calculate_ssim is not None:
            per_pattern[pattern_type]["ssim"].append(calculate_ssim(pred_img, target_img, crop_border=0, input_order="HWC"))

        del inp, target, pred

    summary = {}
    for pattern_type, metrics in per_pattern.items():
        mean_psnr = float(np.mean(metrics["psnr"])) if metrics["psnr"] else float("nan")
        mean_ssim = float(np.mean(metrics["ssim"])) if metrics["ssim"] else float("nan")
    ...
    balanced_psnr = float(np.mean([metrics["psnr"] for metrics in summary.values()])) if summary else float("nan")
    balanced_ssim = float(np.mean([metrics["ssim"] for metrics in summary.values()])) if summary else float("nan")
    clear_cuda_memory(device)
    return {
        "balanced_psnr": balanced_psnr,
        "balanced_ssim": balanced_ssim,
        "per_pattern": summary,
    }
...
def print_validation_metrics(metrics: Dict):
    print(f"Validation Balanced: PSNR={metrics['balanced_psnr']:.4f} dB, SSIM={metrics['balanced_ssim']:.4f}")
    for pattern_type in ["single", "quad", "nona"]:
        if pattern_type not in metrics["per_pattern"]:
            continue
        pattern_metrics = metrics["per_pattern"][pattern_type]
        print(
            f"  {pattern_type}: "
            f"PSNR={pattern_metrics['psnr']:.4f} dB, "
            f"SSIM={pattern_metrics['ssim']:.4f}, "
            f"N={pattern_metrics['count']}"
        )
...
def save_training_curves(save_dir: Path, history: Dict[str, List]):
    ...
    _save_line_plot_svg(
        ...
        epochs=epochs,
        series=[
            ("balanced", history["balanced_psnr"], "#111827"),
            ("single", history["single_psnr"], "#dc2626"),
            ("quad", history["quad_psnr"], "#059669"),
            ("nona", history["nona_psnr"], "#7c3aed"),
        ],
        y_tick_format="fixed",
    )
...
def append_history(
    ...
    epoch_seconds: float,
    metrics: Dict | None,
    pattern_sample_counts: Dict[str, int],
):
    history["epoch"].append(int(epoch))
    history["train_loss"].append(float(train_loss))
    history["epoch_seconds"].append(float(epoch_seconds))
    history["pattern_sample_counts"].append({key: int(value) for key, value in sorted(pattern_sample_counts.items())})

    if metrics is None:
        history["balanced_psnr"].append(float("nan"))
        history["balanced_ssim"].append(float("nan"))
        for pattern_type in ["single", "quad", "nona"]:
            history[f"{pattern_type}_psnr"].append(float("nan"))
            history[f"{pattern_type}_ssim"].append(float("nan"))
        return

    history["balanced_psnr"].append(float(metrics["balanced_psnr"]))
    history["balanced_ssim"].append(float(metrics["balanced_ssim"]))
    for pattern_type in ["single", "quad", "nona"]:
        pattern_metrics = metrics["per_pattern"].get(pattern_type)
        if pattern_metrics is None:
            history[f"{pattern_type}_psnr"].append(float("nan"))
            history[f"{pattern_type}_ssim"].append(float("nan"))
        else:
            history[f"{pattern_type}_psnr"].append(float(pattern_metrics["psnr"]))
            history[f"{pattern_type}_ssim"].append(float(pattern_metrics["ssim"]))
...
def main():
    ...
    if args.train_pattern_weights is not None and len(args.train_pattern_weights) not in (0, len(args.train_pattern_types)):
        raise ValueError("--train-pattern-weights must be omitted or match --train-pattern-types length.")
    if args.noise_std_min > args.noise_std_max:
        raise ValueError("--noise-std-min must be <= --noise-std-max")
    if args.maskout_p_min > args.maskout_p_max:
        raise ValueError("--maskout-p-min must be <= --maskout-p-max")

    aligned_crop_size = align_crop_size(args.crop_size, args.train_pattern_types, args.window_size)
    if aligned_crop_size != args.crop_size:
        print(f"Adjust crop_size from {args.crop_size} to {aligned_crop_size} to match pattern/window alignment.")
    ...
    print(f"Train pattern types: {args.train_pattern_types}")
    print(f"Val pattern types: {args.val_pattern_types}")
    print(f"Base Bayer pattern: {args.base_pattern}")
    print(f"Maskout enabled: {args.maskout_enabled}")
    print("Pattern sampling: random per sample from train_pattern_types")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        epoch_pattern_counts = defaultdict(int)

        for step, batch in enumerate(train_loader, start=1):
            inp = batch["input"].to(device, non_blocking=True)
            target = batch["target"].to(device, non_blocking=True)
            batch_pattern_types = batch["pattern_type"]
            if isinstance(batch_pattern_types, str):
                epoch_pattern_counts[batch_pattern_types] += 1
            else:
                for pattern_type in batch_pattern_types:
                    epoch_pattern_counts[str(pattern_type)] += 1
        ...
        print(
            "Pattern sample counts this epoch: "
            + ", ".join(f"{pattern_type}={epoch_pattern_counts.get(pattern_type, 0)}" for pattern_type in ["single", "quad", "nona"])
        )

        metrics = None
        if val_loader is not None and epoch % args.val_every == 0:
            del inp, target, pred, loss, batch, batch_pattern_types
            clear_cuda_memory(device)
            metrics = validate(model, val_loader, device)
            print_validation_metrics(metrics)
            if metrics["balanced_psnr"] > best_psnr:
                best_psnr = metrics["balanced_psnr"]
                save_checkpoint(save_dir / "model_best.pth", epoch, model, optimizer, scheduler, best_psnr)
                print(f"Saved new best checkpoint with balanced PSNR {best_psnr:.4f} dB")
        ...
        append_history(
            ...
            metrics=metrics,
            pattern_sample_counts=epoch_pattern_counts,
        )
