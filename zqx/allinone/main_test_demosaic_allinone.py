...
from data.demosaic_allinone_dataset import VALID_BASE_PATTERNS, build_allinone_input
from data.demosaic_rgb_dataset import collect_image_paths, ensure_pattern_size, imread_rgb_float, raw_and_mask_to_mosaic_visual
from models.network_swinir import SwinIR


def parse_args():
    parser = argparse.ArgumentParser(description="Visualize all-in-one SwinIR demosaicing results.")
    ...
    parser.add_argument("--output-dir", type=str, default="results/demosaic_allinone_preview")
    parser.add_argument("--pattern-type", type=str, default="", choices=["", "single", "quad", "nona"])
    parser.add_argument("--base-pattern", type=str, default="", choices=["", *VALID_BASE_PATTERNS])
...
def build_model_from_config(config: Dict, device: torch.device) -> SwinIR:
    model = SwinIR(
        img_size=config.get("crop_size", 144),
        patch_size=1,
        in_chans=4,
        out_chans=3,
...
def infer_one(
    ...
    gt = ensure_pattern_size(gt, pattern_type)
    inp, raw_mosaic, pattern_mask = build_allinone_input(
        gt,
        pattern_type=pattern_type,
        base_pattern=base_pattern,
        noise_std_range=(noise_std, noise_std),
        maskout_enabled=False,
        maskout_prob_range=(0.0, 0.0),
    )

    pred = model(numpy_to_tensor(inp).to(device))[0]

    gt_u8 = np.round(gt * 255.0).astype(np.uint8)
    mosaic_u8 = np.round(np.clip(raw_and_mask_to_mosaic_visual(raw_mosaic, pattern_mask), 0, 1) * 255.0).astype(np.uint8)
...
def build_panel(
    ...
    tiles = [
        add_title(gt, "GT"),
        add_title(mosaic, f"Mosaic ({pattern_type})"),
...
def main():
    ...
    pattern_type = args.pattern_type or config.get("val_pattern_types", ["single"])[0]
    base_pattern = args.base_pattern or config.get("base_pattern", "RGGB")
    ...
    print(f"Using pattern type: {pattern_type}")
    print(f"Using base pattern: {base_pattern}")
    ...
    for image_path in image_paths:
        ...
        panel = build_panel(gt, mosaic, pred, error, args.error_scale, pattern_type)
